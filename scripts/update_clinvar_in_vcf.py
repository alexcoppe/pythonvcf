...
def update_clinvar_column(original_line_from_vcf, field, clinvar_entry):
    ...
    if clinvar_match:
        ...
        new_match = clinvar_entry[d.get(field)]
...
def update_variant_from_vcf(original_line_from_vcf, clinvar_entry):
    updated_clnsig =  update_clinvar_column(original_line_from_vcf, "CLNSIG", clinvar_entry)[:-1]
    #print(updated_clnsig)
    updated_clnrevstat =  update_clinvar_column(updated_clnsig, "CLNREVSTAT", clinvar_entry)[:-1]
    #print(updated_CLNREVSTAT)
    return updated_clnrevstat
                ...
                if match:
                    ...
                    if len(rows) == 0:
                        ...
                        if description_from_vcs != descrition_from_clinvar:
                            new_variant = update_variant_from_vcf(line, rows[0])
                            print(new_variant)
            else:
                print(line[:-1])
